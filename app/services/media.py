...
async def stage_upload(upload: UploadFile, settings: Settings) -> tuple[Path, int, str, str]:
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=suffix, dir=settings.storage_root) as temporary:
        while chunk := await upload.read(1024 * 1024):
            total += len(chunk)
            if total > settings.max_upload_bytes:
                temporary.close()
                Path(temporary.name).unlink(missing_ok=True)
                raise MediaValidationError("The uploaded file exceeds the configured size limit.")
...
def inspect_media(path: Path, kind: str) -> tuple[float | None, int | None, int | None]:
    ...
    try:
        ...
        if not success or width <= 0 or height <= 0:
            raise MediaValidationError("The uploaded video contains no readable frames.")
        return (frame_count / fps if fps > 0 else None), width, height
    finally:
        capture.release()
